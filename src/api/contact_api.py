...
async def get_all_or_search_contacts(
    ...
    contacts =  await contact_service.get_all_contacts(user, session, first_name, last_name, email)
    return [ContactResponse.model_validate(c) for c in contacts]


# Create new contact
@router.post("/contacts/", response_model=ContactResponse)
async def create_contact(contact: ContactRequest, session: AsyncSession = Depends(open_session), user: User = Depends(get_current_user)):
...
async def get_upcoming_birthdays(session: AsyncSession = Depends(open_session), user: User = Depends(get_current_user)):
...
async def get_contact_by_id(contact_id: int, session: AsyncSession = Depends(open_session), user: User = Depends(get_current_user)):
...
async def update_contact(
    ...
    updated_contact = await contact_service.update_contact(session, db_contact, contact, user)
    return ContactResponse.model_validate(updated_contact)


@router.delete("/contacts/{contact_id}")
async def delete_contact(contact_id: int, session: AsyncSession = Depends(open_session), user: User = Depends(get_current_user)):
